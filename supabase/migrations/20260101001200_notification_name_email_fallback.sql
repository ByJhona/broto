...
begin
  ...
  if post_owner != new.user_id then
    ...
    if liker_name is null or liker_name = '' then
      select split_part(email, '@', 1) into liker_name from auth.users where id = new.user_id;
    end if;
...
begin
  ...
  if post_owner != new.user_id then
    ...
    if commenter_name is null or commenter_name = '' then
      select split_part(email, '@', 1) into commenter_name from auth.users where id = new.user_id;
    end if;
